...
//import labelingStudy.nctu.boredom_detection.question.QuestionConfig;
...
//        Model tagsModel = Model.getInstance();
        ...
        /*AsyncTask.execute(new Runnable() {
            ...
            public void run() {
                ...
                //
                try {
                    Log.i(LOG_TAG, "initialize: getting mUserSubmissionStats from future ");
                    ...
                    if(!areDatesEqual((new Date().getTime()), mUserSubmissionStats.getCreationTime())
                        ...
                        if(mUserSubmissionStats == null)
                            Log.i(LOG_TAG, "initialize: userSubmissionStats is null");
                        Log.i(LOG_TAG, "initialize: userSubmissionStats is either null or we have a new date." +
                    ...
                    e.printStackTrace();
                    Log.i(LOG_TAG, "initialize: Creating mUserSubmissionStats");
    ...
    public UserSubmissionStats getUserSubmissionStats() {
            if((mUserSubmissionStats == null) || !areDatesEqual((new Date().getTime()), mUserSubmissionStats.getCreationTime())) {
                if(mUserSubmissionStats == null)
                    Log.i(LOG_TAG, "getUserSubmissionStats: userSubmissionStats is null");

                Log.i(LOG_TAG, "getUserSubmissionStats: userSubmissionStats is either null or we have a new date." +
                                "Creating new userSubmissionStats object");
            mUserSubmissionStats = new UserSubmissionStats();
        }
    ...
    protected boolean areDatesEqual(long currentTime, long previousTime) {
        Log.i(LOG_TAG, "Checking if the both dates are the same");
        ...
        Log.i(LOG_TAG, "Current Year:" + currentDate.get(Calendar.YEAR) + " Previous Year:" + previousDate.get(Calendar.YEAR));
        Log.i(LOG_TAG, "Current Day:" + currentDate.get(Calendar.DAY_OF_YEAR) + " Previous Day:" + previousDate.get(Calendar.DAY_OF_YEAR));
        Log.i(LOG_TAG, "Current Month:" + currentDate.get(Calendar.MONTH) + " Previous Month:" + previousDate.get(Calendar.MONTH));
        ...
        if(sameDay)
            Log.i(LOG_TAG, "it is the same day, should not create a new object");
        else
            Log.i(LOG_TAG, "it is not the same day - a new day, should create a new object");
        return sameDay;
    }
}
